...
# list of switches that are applying a VLAN tag
tagging_switches = {}

# global VLAN id
global_next_vlan_id= 110
...
ADD_PCOUNT_TAG_FLAG=True



def _get_next_vlad_id ():
  """
  DPG: return the next vlan_id number.  the vlan_numbers are globally unique
  """
  global global_next_vlan_id 
  
  global_next_vlan_id = global_next_vlan_id + 1

  return global_next_vlan_id
...
class Switch (EventMixin):
  ...
  def _add_pcount_match_rule(self, switch, msg, match):
    
    msg.match.dl_vlan = 111

    print " *******************************************************************"     
    print " *** TODO: _add_pcount_match_rule for Switch = %s ***" %(switch)
    print " *******************************************************************"

  def _add_pcount_tag(self, switch, msg, match):
    
    vlan_vid = _get_next_vlad_id()
    msg.actions.append(of.ofp_action_vlan_vid(vlan_vid = vlan_vid))
    
    self.tag_flow_map[vlan_vid] = msg
    
    print " **********************************************************************"
    print "switch = %s, VLAN = %s" %(switch,vlan_vid)
    print " ********************************************************************** "
    
    return vlan_vid
    
  def _install (self, switch, port, match, buf = -1, is_tag_switch=False):
    
    # DPG -- add my code here for tagging packets
    msg = of.ofp_flow_mod()
    ...
    vlan_vid = -1
    if ADD_PCOUNT_TAG_FLAG:
#      msg.match = of.ofp_match(dl_type = ethernet.IP_TYPE, dl_src=match._dl_src, dl_dst = match._dl_dst) #DPG: match using L2 address
      msg.match = of.ofp_match(dl_type = ethernet.IP_TYPE, nw_src=match._nw_src, nw_dst = match._nw_dst) #DPG: match using L3 address

      if is_tag_switch:
        vlan_vid = self._add_pcount_tag(switch,msg,match)
      else: 
        self._add_pcount_match_rule(switch,msg,match)
      print "DPG:    %s" %(msg.match)
    else:
        msg.match = match #DPG: creates flow using exact settings of packet
  ...
  def _install_path (self, p, match, buffer_id = -1):
    ...
    path_nodes = []
    for sw,port in p[1:]:
      self._install(sw, port, match,False)   #DPG -- investigate this line
    ...
    if ADD_PCOUNT_TAG_FLAG:
      ...
      vlan_vid = self._install(p[0][0], p[0][1], match, buffer_id,is_tag_switch)
      
      if vlan_vid !=-1:
        # DPG: add the tagging node to the front of the list
        path_nodes.insert(1, p[0][0])
        tagging_switches[vlan_vid] = path_nodes
    
    else:
      self._install(p[0][0], p[0][1], match, buffer_id,True)
    
    print "====================================================================================================================================== \n"

    #core.l2_multi.raiseEvent(PathInstalled(p))
    core.l2_multi_dpg.raiseEvent(PathInstalled(p))
  ...
  def _handle_PacketIn (self, event):
    ...
    if packet.dst.isMulticast():
      ...
      if packet.dst not in mac_map:
        ...
        match = of.ofp_match.from_packet(packet) #DPG: this is where the flow match rule is taken directly from the packet
      #  print "** DPG: calling self.install_path(dest[0]=%s,dest[1]=%s,martch=...,event=%s)  ** " %(dest[0], dest[1], event)
        self.install_path(dest[0], dest[1], match, event) #DPG R1
